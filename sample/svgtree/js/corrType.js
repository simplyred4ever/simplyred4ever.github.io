...
function CorrType(typeArray, colorArray) {
	...
	this.svg = new $$.Group("type_g", document.documentElement).position(width - this.width, height - 30 - this.height).addClass('DragCorrType');
	this.rect =  new $$.Rect("type_rect", this.svg.self).bound(0, 0, this.width, this.height).addClass('typeFill defBorder');
	this.text =  new $$.Text("type_title", this.svg.self).position(paddingL, 20).text("关联类型").addClass('text');

	for(var i = 0; i < this.typeArray.length; i++){
		this.lineColor[this.typeArray[i]] = colorArray[i % 10];
		new CorrType.TypeLine('typeLine' + i, this.svg.self, typeArray[i], colorArray[i % 10], i);
	}
...
CorrType.DragCorrType = function (evt) {
	...
	this.x = evt.clientX;
	this.y = evt.clientY;
	var position = corrType.svg.position();
	this.tx = position.x;
	this.ty = position.y;
}
...
CorrType.DragCorrType.prototype = {
	drag : function (evt) {
		...
		y = Math.min(window.innerHeight - 30 - corrType.height, Math.max(0, y));
		corrType.svg.position(x, y);
	},
	drop : function () {
