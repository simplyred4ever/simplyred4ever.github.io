...
(function () {
		...
		a.env = (function () {
			...
			if (d.ie) {
				...
				if (document.documentMode && document.documentMode >= 9)
					d.cssClass += ' cke_browser_ie9plus';
			}
			if (d.gecko && e < 10900)
	...
	(function () {
		...
		a.tools = {
			...
			normalizeCssText : function (f, g) {
				var h = [],
				i,
				j = a.tools.parseCssText(f, true, g);
				for (i in j)
					h.push(i + ':' + j[i]);
				h.sort();
			...
			convertRgbToHex : function (f) {
				return f.replace(/(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi, function (g, h, i, j) {
					var k = [h, i, j];
					for (var l = 0; l < 3; l++)
						k[l] = ('0' + parseInt(k[l], 10).toString(16)).slice(-2);
					return '#' + k.join('');
				});
			},
			parseCssText : function (f, g, h) {
				...
				f.replace(/&quot;/g, '"').replace(/\s*([^:;\s]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function (k, l, m) {
					if (g) {
						l = l.toLowerCase();
						if (l == 'font-family')
							m = m.toLowerCase().replace(/["']/g, '').replace(/\s*,\s*/g, ',');
						m = a.tools.trim(m);
					}
					i[l] = m;
				});
	...
	(function () {
		e.extend(h.prototype, {
			...
			appendText : function (l) {
				if (this.$.text != undefined)
					...
					this.append(new d.text(l));
			},
			appendBogus : function () {
				var n = this;
				var l = n.getLast();
				while (l && l.type == 3 && !e.rtrim(l.getText()))
					l = l.getPrevious();
				if (!l || !l.is || !l.is('br')) {
			...
			getTabIndex : c ? function () {
				var l = this.$.tabIndex;
				if (l === 0 && !f.$tabIndex[this.getName()] && parseInt(this.getAttribute('tabindex'), 10) !== 0)
					l = -1;
				return l;
			}
			 : b.webkit ? function () {
			...
			isEditable : function (l) {
				var o = this;
				var m = o.getName();
				if (o.isReadOnly() || o.getComputedStyle('display') == 'none' || o.getComputedStyle('visibility') == 'hidden' || o.is('a') && o.data('cke-saved-name') && !o.getChildCount() || f.$nonEditable[m] || f.$empty[m])
					return false;
				if (l !== false) {
					var n = f[m] || f.span;
					return n && n['#'];
			...
			isIdentical : function (l) {
				...
				for (var q = 0; q < o; q++) {
					var r = m[q];
					if (r.nodeName == '_moz_dirty')
						continue;
					if ((!c || r.specified && r.nodeName != 'data-cke-expando') && r.nodeValue != l.getAttribute(r.nodeName))
						return false;
				}
				if (c)
					for (q = 0; q < p; q++) {
						r = n[q];
						if (r.specified && r.nodeName != 'data-cke-expando' && r.nodeValue != this.getAttribute(r.nodeName))
							return false;
					}
				return true;
			},
			isVisible : function () {
				...
				if (l && (b.webkit || b.opera)) {
					m = o.getWindow();
					if (!m.equals(a.document.getWindow()) && (n = m.$.frameElement))
						l = new h(n).isVisible();
				}
				return !!l;
			},
			isEmptyInlineRemoveable : function () {
				...
				for (var m = 0, n = l.count(); m < n; m++) {
					var o = l.getItem(m);
					if (o.type == 1 && o.data('cke-bookmark'))
						continue;
					if (o.type == 1 && !o.isEmptyInlineRemoveable() || o.type == 3 && e.trim(o.getText()))
						return false;
				}
				return true;
			},
			...
			mergeSiblings : (function () {
				...
				return function (m) {
					var n = this;
					if (!(m === false || f.$removeEmpty[n.getName()] || n.is('a')))
						return;
					l(n, n.getNext(), true);
					l(n, n.getPrevious());
				};
				...
				if (c && (b.ie7Compat || b.ie6Compat))
					return function (m, n) {
						...
						if (m == 'class')
							...
							o.$.checked = n;
						else if (m == 'contenteditable')
							l.call(o, 'contentEditable', n);
						else
							l.apply(o, arguments);
						return o;
					};
			...
			removeAttributes : function (l) {
				if (e.isArray(l))
					...
					for (var n in l)
						l.hasOwnProperty(n) && this.removeAttribute(n);
			},
			...
			scrollIntoParent : function (l, m, n) {
				...
				if (l instanceof d.window)
					l = p ? o.getBody() : o.getDocumentElement();
				function q(C, D) {
					if (/body|html/.test(l.getName()))
						l.getWindow().$.scrollBy(C, D);
					else {
						l.$.scrollLeft += C;
						l.$.scrollTop += D;
					}
				...
				if (A.y < 0 || B.y > 0)
					q(0, m === true ? A.y : m === false ? B.y : A.y < 0 ? A.y : B.y);
				if (n && (A.x < 0 || B.x > 0))
					q(A.x < 0 ? A.x : B.x, 0);
			},
			setState : function (l) {
			...
			copyAttributes : function (l, m) {
				...
				for (var o = 0; o < n.length; o++) {
					...
					if (q in m)
						continue;
					if (q == 'checked' && (r = s.getAttribute(q)))
						l.setAttribute(q, r);
			...
			disableContextMenu : function () {
				this.on('contextmenu', function (l) {
					if (!l.data.getTarget().hasClass('cke_enable_context_menu'))
						l.data.preventDefault();
				});
			},
		...
		function k(l) {
			var m = 0;
			for (var n = 0, o = i[l].length; n < o; n++)
				m += parseInt(this.getComputedStyle(i[l][n]) || 0, 10) || 0;
			return m;
		};
	...
	(function () {
		...
		d.walker.whitespaces = function (s) {
			return function (t) {
				var u;
				if (t && t.type == 3)
					u = !e.trim(t.getText()) || b.webkit && t.getText() == '​';
				return !!(s^u);
			};
		};
	...
	(function () {
		...
		m = function (v, w, x, y) {
			...
			for (var P = H; P < F.length; P++) {
				...
				N = L.getNext();
				while (N) {
					if (N.equals(G[P]) || N.equals(A))
						break;
					O = N.getNext();
					if (w == 2)
						K.append(N.clone(true));
					else {
						N.remove();
						if (w == 1)
							K.append(N);
			...
			for (var Q = H; Q < G.length; Q++) {
				...
				if (!F[Q] || L.$.parentNode != F[Q].$.parentNode) {
					N = L.getPrevious();
					while (N) {
						if (N.equals(F[Q]) || N.equals(z))
							break;
						O = N.getPrevious();
						if (w == 2)
							K.$.insertBefore(N.$.cloneNode(true), K.$.firstChild);
						else {
							N.remove();
							if (w == 1)
			...
			if (w == 2) {
				...
				if (I && J && (z.$.parentNode != I.$.parentNode || A.$.parentNode != J.$.parentNode)) {
					var T = J.getIndex();
					if (D && J.$.parentNode == z.$.parentNode)
						T--;
					if (y && I.type == 1) {
		...
		function o() {
			...
			return function (z) {
				...
				if (z.type == 3 && (z.hasAscendant('pre') || e.trim(z.getText()).length))
					return false;
				if (z.type == 1 && !n[z.getName()])
					return false;
				return true;
			};
		};
		...
		d.range.prototype = {
			...
			getBoundaryNodes : function () {
				...
				if (w.type == 1) {
					z = w.getChildCount();
					if (z > y)
						w = w.getChild(y).getPreviousSourceNode(true);
					else if (z < 1)
						w = w.getPreviousSourceNode();
					else {
						w = w.$;
						while (w.lastChild)
							w = w.lastChild;
			...
			getCommonAncestor : function (v, w) {
				...
				z;
				if (x.equals(y)) {
					if (v && x.type == 1 && A.startOffset == A.endOffset - 1)
						z = x.getChild(A.startOffset);
					else
						z = x;
				} else
					z = x.getCommonAncestor(y);
			...
			optimizeBookmark : function () {
				...
				var v = x.startContainer,
				w = x.endContainer;
				if (v.is && v.is('span') && v.data('cke-bookmark'))
					x.setStartAt(v, 3);
				if (w && w.is && w.is('span') && w.data('cke-bookmark'))
					x.setEndAt(w, 4);
			},
			trim : function (v, w) {
				var D = this;
				...
				if ((!v || z) && x && x.type == 3) {
					if (!y) {
						...
						y = x.getIndex() + 1;
						x = x.getParent();
						if (D.startContainer.equals(D.endContainer))
							D.setEnd(A, D.endOffset - D.startOffset);
						else if (x.equals(D.endContainer))
							D.endOffset += 1;
					}
					D.setStart(x, y);
					if (z) {
			...
			insertNode : function (v) {
				...
				if (y)
					...
					w.append(v);
				if (v.getParent().equals(z.endContainer))
					z.endOffset++;
				z.setStartBefore(v);
			},
			...
			setStart : function (v, w) {
				var x = this;
				if (v.type == 1 && f.$empty[v.getName()])
					w = v.getIndex(), v = v.getParent();
				x.startContainer = v;
				x.startOffset = w;
				if (!x.endContainer) {
					x.endContainer = v;
			...
			setStartAt : function (v, w) {
				...
				case 2:
					if (v.type == 3)
						x.setStart(v, v.getLength());
					else
						x.setStart(v, v.getChildCount());
					break;
				case 3:
					x.setStartBefore(v);
					break;
			...
			setEndAt : function (v, w) {
				...
				case 2:
					if (v.type == 3)
						x.setEnd(v, v.getLength());
					else
						x.setEnd(v, v.getChildCount());
					break;
				case 3:
					x.setEndBefore(v);
					break;
			...
			checkReadOnly : (function () {
				function v(w, x) {
					while (w) {
						if (w.type == 1)
							if (w.getAttribute('contentEditable') == 'false' && !w.data('cke-editable'))
								return 0;
							else if (w.is('html') || w.getAttribute('contentEditable') == 'true' && (w.contains(x) || w.equals(x)))
								break;
						w = w.getParent();
					}
					return 1;
			...
			moveToElementEditablePosition : function (v, w) {
				function x(z, A) {
					...
					if (z.type == 1 && z.isEditable(false))
						B = z[w ? 'getLast' : 'getFirst'](u);
					if (!A && !B)
						B = z[w ? 'getPrevious' : 'getNext'](u);
					return B;
				};
				...
				while (v) {
					if (v.type == 3) {
						if (w && this.checkEndOfBlock() && t.test(v.getText()))
							this.moveToPosition(v, 3);
						else
							this.moveToPosition(v, w ? 4 : 3);
						y = 1;
						break;
					...
					if (v.type == 1)
						if (v.isEditable()) {
							...
							y = 1;
						} else if (w && v.is('br') && this.checkEndOfBlock())
							this.moveToPosition(v, 3);
					v = x(v, y);
				}
				return !!y;
			...
			getEnclosedNode : function () {
				var v = this.clone();
				v.optimize();
				if (v.startContainer.type != 1 || v.endContainer.type != 1)
					return null;
					...
					if (p.styles)
						s.push({
							...
							getStyle : function (t, u) {
								var v = this.getValue().match(new RegExp('(?:^|;)\\s*' + t + '\\s*:\\s*([^;]*)', 'i'));
								return v ? v[1] : u;
							},
	...
	(function () {
		...
		function o(U) {
			...
			W = U.children[V - 1];
			while (W && W.type == 3 && !e.trim(W.value))
				W = U.children[--V];
			return W;
		};
		function p(U) {
		...
		function q(U, V) {
			...
			X = o(U);
			if (X) {
				if ((V || !c) && X.type == 1 && X.name == 'br')
					W.pop();
				if (X.type == 3 && m.test(X.value))
					W.pop();
			}
		};
		function r(U, V, W) {
			if (!V && (!W || typeof W == 'function' && W(U) === false))
				return false;
			if (V && c && (document.documentMode > 7 || U.name in f.tr || U.name in f.$listItem))
				return false;
			var X = o(U);
		...
		function s(U, V) {
			return function (W) {
				...
				if (r(W, !U, V))
					if (U || c)
						W.add(new a.htmlParser.text('\xa0'));
					else
						W.add(new a.htmlParser.element('br', {}));
			};
		};
		var t = f,
		...
		var z = {
			...
			elements : {
				...
				a : function (U) {
					if (!(U.children.length || U.attributes.name || U.attributes['data-cke-saved-name']))
						return false;
				},
				span : function (U) {
					if (U.attributes['class'] == 'Apple-style-span')
						delete U.name;
				},
				pre : function (U) {
					c && q(U);
				...
				style : function (U) {
					...
					if (!U.attributes.type)
						U.attributes.type = 'text/css';
				},
				title : function (U) {
					var V = U.children[0];
		...
		if (c)
			z.attributes.style = function (U, V) {
		...
		function A(U) {
			var V = U.attributes;
			if (V.contenteditable != 'false')
				V['data-cke-editable'] = V.contenteditable ? 'true' : 1;
		...
		function J(U) {
			return U.replace(C, function (V, W, X) {
				return '<' + W + X.replace(D, function (Y, Z) {
					if (!/^on/.test(Z) && X.indexOf('data-cke-saved-' + Z) == -1)
						return ' data-cke-saved-' + Y + ' data-cke-' + a.rnd + '-' + Y;
					return Y;
				}) + '>';
			});
		};
		...
		j.add('htmldataprocessor', {
			...
			init : function (U) {
				...
				for (w in v)
					W.elements[w] = s(true, U.config.fillEmptyBlocks);
	...
	(function () {
		...
		function L(N, O, P) {
			...
			while (P.checkStartOfBlock() && P.checkEndOfBlock()) {
				...
				if (ac.is('li')) {
					ad = ac.getParent();
					if (ac.equals(ad.getLast(q)) && ac.equals(ad.getFirst(q)))
						ac = ad;
				}
			...
			var ah = ag.next();
			if (ah && ah.type == 1 && ah.getName()in f.$list)
				C(ah);
			O.moveToBookmark(R);
			O.select();
			N.selectionChange(1);
						...
						if (V) {
							...
							if (ac) {
								...
								if (ad && ad.type == 1 && ad.getName()in m && ad.equals(af)) {
									ah = 1;
									ad = X.next();
								} else if (T.checkBoundaryOfElement(ag, 2))
									ah = 1;
								if (ah && ad) {
									ae = T.clone();
...
(function () {
	...
	var p = function () {
		var t = this,
		u = function () {
			if (q.instances[t.name])
				q.instances[t.name].destroy();
			var y = t.config,
			z = {};
		...
		t.on('beforeCommandExec', function (y) {
			if (y.data.name == 'source' && t.mode == 'source')
			...
			if (t.contextMenu && t.addMenuItems)
				t.contextMenu.addListener(function (E, F) {
					...
					if (!I)
						return null;
					var J = G.getLang(),
					K = window.scayt.getSuggestion(I, J);
					for (var L in w) {
						delete t._.menuItems[L];
					...
					w = {};
					x = {};
					var M = t.config.scayt_moreSuggestions || 'on',
					N = false,
					O = t.config.scayt_maxSuggestions;
					typeof O != 'number' && (O = 5);
					!O && (O = K.length);
					var P = t.config.scayt_contextCommands || 'all';
					P = P.split('|');
					if (K && K.length) {
						for (var Q = 0, R = K.length; Q < R; Q += 1) {
							var S = 'scayt_suggestion_' + K[Q].replace(' ', '_'),
							T = (function (X, Y) {
								return {
									exec : function () {
										G.replace(X, Y);
							...
							if (Q < O) {
								r(t, 'button_' + S, K[Q], S, T, 'scayt_suggest', Q + 1);
								x[S] = 2;
							} else if (M == 'on') {
								r(t, 'button_' + S, K[Q], S, T, 'scayt_moresuggest', Q + 1);
								w[S] = 2;
								N = true;
							}
						}
						...
						t.addMenuItem('scayt_nosuggest', {
							label : t.lang.scayt.noSuggestions ? t.lang.scayt.noSuggestions : t.lang.spellCheck.noSuggestions ? t.lang.spellCheck.noSuggestions : 'No suggestions',
							group : 'scayt_suggest',
							order : 1
						});
						x.scayt_nosuggest = 2;
					}
					if (o('all', P) || o('ignore', P)) {
						var U = {
							exec : function () {
								G.ignore(H);
							}
...
i.toolbar_Full = [{
		...
		name : 'clipboard',
		items : ['Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', 'PasteImage', '-', 'Undo', 'Redo']
	}, {
		name : 'editing',
...
(function () {
	...
	function o() {
		...
		for (var U = T - 1; U > T - S; U--) {
			if (n.call(V, V._.tabIdList[U % S]))
				return V._.tabIdList[U % S];
		}
		return null;
	};
	function p() {
	...
	function r(R, S) {
		...
		if (T)
			R ? T.removeAttribute('aria-invalid') : T.setAttribute('aria-invalid', true);
		if (!R)
			if (U.select)
				U.select();
			else
	...
	a.dialog = function (R, S) {
		...
		Z,
		aa;
		if (V == 'OS' && b.mac || V == 'rtl' && W == 'ltr' || V == 'ltr' && W == 'rtl')
			U.buttons.reverse();
		T = e.extend(T(R), U);
		T = e.clone(T);
		T = new z(this, T);
		...
		if (R._.removeDialogTabs && (X = R._.removeDialogTabs[S]))
			for (Y = 0; Y < X.length; Y++)
				T.removeContents(X[Y]);
		if (T.onLoad)
			this.on('load', T.onLoad);
		if (T.onShow)
			this.on('show', T.onShow);
		if (T.onHide)
			this.on('hide', T.onHide);
		if (T.onOk)
			this.on('ok', function (ar) {
				...
				if (T.onOk.call(this, ar) === false)
					ar.data.hide = false;
			});
		if (T.onCancel)
			this.on('cancel', function (ar) {
				if (T.onCancel.call(this, ar) === false)
					ar.data.hide = false;
			});
		var ah = this,
		ai = function (ar) {
		...
		this.on('cancel', function (ar) {
			ai(function (as) {
				if (as.isChanged()) {
					if (!confirm(R.lang.common.confirmCancel))
						ar.data.hide = false;
					return true;
				}
			});
		...
		function aj() {
			...
			ar.sort(function (au, av) {
				if (au.tabIndex != av.tabIndex)
					return av.tabIndex - au.tabIndex;
				else
					return au.focusIndex - av.focusIndex;
			});
			var as = ar.length;
		...
		function am(ar) {
			if (Z)
				ar.data.preventDefault(1);
			else if (aa)
				ar.data.stopPropagation();
		};
		var an = this._.element;
		this.on('show', function () {
			an.on('keydown', al, this);
			if (b.opera || b.gecko)
				an.on('keypress', am, this);
		});
		this.on('hide', function () {
			an.removeListener('keydown', al);
			if (b.opera || b.gecko)
				an.removeListener('keypress', am);
			ai(function (ar) {
				s.apply(ar);
			});
		});
		...
		for (Y = 0; Y < aq.length; Y++)
			this._.buttons[aq[Y].id] = aq[Y];
	};
	function t(R, S, T) {
		this.element = S;
		this.focusIndex = T;
	...
	a.dialog.prototype = {
		...
		resize : (function () {
			return function (R, S) {
				var T = this;
				if (T._.contentSize && T._.contentSize.width == R && T._.contentSize.height == S)
					return;
				...
				if (T._.editor.lang.dir == 'rtl' && T._.position)
					T._.position.x = a.document.getWindow().getViewPaneSize().width - T._.contentSize.width - parseInt(T._.element.getFirst().getStyle('right'), 10);
				T._.contentSize = {
					width : R,
					height : S
				};
		...
		move : function (R, S, T) {
			...
			U.setStyle('zoom', '100%');
			if (W && ab._.position && ab._.position.x == R && ab._.position.y == S)
				return;
			ab._.position = {
				x : R,
				y : S
		...
		show : function () {
			...
			S = this.definition;
			if (!(R.getParent() && R.getParent().equals(a.document.getBody())))
		...
		layout : function () {
			...
			if (!b.ie6Compat)
				if (S.height + (W > 0 ? W : 0) > U.height || S.width + (V > 0 ? V : 0) > U.width)
		...
		foreach : function (R) {
			...
			for (var S in U._.contents)
				for (var T in U._.contents[S])
					R.call(U, U._.contents[S][T]);
			return U;
		},
		reset : (function () {
			var R = function (S) {
		...
		commitContent : function () {
			var R = arguments;
			this.foreach(function (S) {
				if (c && this._.currentFocusIndex == S.focusIndex)
		...
		addPage : function (R) {
			...
			while (ab = ac.shift()) {
				aa[ab.id] = ab;
				if (typeof ab.getChild == 'function')
					ac.push.apply(ac, ab.getChild());
			}
		...
		hidePage : function (R) {
			...
			if (!S || T._.pageCount == 1 || !S.isVisible())
				return;
			else if (R == T._.currentTabId)
				T.selectPage(o.call(T));
			S.hide();
			T._.pageCount--;
			T.updateStyle();
		},
		...
		addFocusable : function (R, S) {
			...
			if (typeof S == 'undefined') {
				...
				for (var T = S + 1; T < U._.focusList.length; T++)
					U._.focusList[T].focusIndex++;
			}
		}
	};
	e.extend(a.dialog, {
		add : function (R, S) {
			if (!this._.dialogDefinitions[R] || typeof S == 'function')
				this._.dialogDefinitions[R] = S;
		},
		exists : function (R) {
	...
	function B(R) {
		...
		function Y(aa) {
			...
			if (T.x + X[3] < W)
				ah = -X[3];
			else if (T.x - X[1] > ac.width - ab.width - W)
				ah = ac.width - ab.width + (V.lang.dir == 'rtl' ? 0 : X[1]);
			else
				ah = T.x;
			if (T.y + X[0] < W)
				ai = -X[0];
			else if (T.y - X[2] > ac.height - ab.height - W)
				ai = ac.height - ab.height + X[2];
			else
				ai = T.y;
			R.move(ah, ai, 1);
	...
	function C(R) {
		...
		R.on('load', function () {
			var ae = '';
			if (T == 1)
				ae = ' cke_resizer_horizontal';
			else if (T == 2)
				ae = ' cke_resizer_vertical';
		...
		function ac(ae) {
			...
			if (ao.y + al > X.height)
				al = X.height - ao.y;
			if ((af ? an : ao.x) + ak > X.width)
				ak = X.width - (af ? an : ao.x);
			if (T == 1 || T == 3)
				ai = Math.max(S.minWidth || 0, ak - V);
			if (T == 2 || T == 3)
				aj = Math.max(S.minHeight || 0, al - W);
			R.resize(ai, aj);
			if (!R._.moved)
				R.layout();
	...
	O = function (R) {
		for (var S in K) {
			var T = K[S];
			for (var U = T.length - 1; U >= 0; U--) {
				if (T[U].dialog == R || T[U].uiElement == R)
					T.splice(U, 1);
			}
			if (T.length === 0)
				delete K[S];
	...
	P = function (R, S) {
		if (R._.accessKeyMap[S])
			R.selectPage(R._.accessKeyMap[S]);
	},
	Q = function (R, S) {};
	(function () {
		k.dialog = {
			uiElement : function (R, S, T, U, V, W, X) {
				...
				for (af in aa)
					aj.push(af + ':' + aa[af]);
				if (S.hidden)
					aj.push('display:none');
				for (af = aj.length - 1; af >= 0; af--) {
					if (aj[af] === '')
						aj.splice(af, 1);
				}
				if (aj.length > 0)
					ab.style = (ab.style ? ab.style + '; ' : '') + aj.join('; ');
				for (af in ab)
					Z.push(af + '="' + e.htmlEncode(ab[af]) + '" ');
				Z.push('>', ac, '</', Y, '>');
				T.push(Z.join(''));
				(this._ || (this._ = {})).dialog = R;
				if (typeof S.isChanged == 'boolean')
					this.isChanged = function () {
						return S.isChanged;
					};
				if (typeof S.isChanged == 'function')
					this.isChanged = S.isChanged;
				if (typeof S.setValue == 'function')
					this.setValue = e.override(this.setValue, function (am) {
							return function (an) {
								am.call(this, S.setValue.call(this, an));
							};
						});
				if (typeof S.getValue == 'function')
					this.getValue = e.override(this.getValue, function (am) {
							return function () {
				...
				if (this.accessKeyUp && this.accessKeyDown && S.accessKey)
					N(this, R, 'CTRL+' + S.accessKey);
				var al = this;
				R.on('load', function () {
			...
			hbox : function (R, S, T, U, V) {
				...
				ab = function () {
					...
					for (aa = 0; aa < T.length; aa++) {
						...
						af = [];
						if (aa === 0)
							ae = 'cke_dialog_ui_hbox_first';
						if (aa == T.length - 1)
							ae = 'cke_dialog_ui_hbox_last';
						ad.push('<td class="', ae, '" role="presentation" ');
						if (X) {
							if (X[aa])
								af.push('width:' + m(X[aa]));
						} else
							af.push('width:' + Math.floor(100 / T.length) + '%');
						if (Y)
							af.push('height:' + m(Y));
						if (V && V.padding != undefined)
							af.push('padding:' + m(V.padding));
						if (c && b.quirks && W[aa].align)
							af.push('text-align:' + W[aa].align);
						if (af.length > 0)
							ad.push('style="' + af.join('; ') + '" ');
						ad.push('>', T[aa], '</td>');
					}
					ad.push('</tr></tbody>');
					return ad.join('');
			...
			vbox : function (R, S, T, U, V) {
				...
				Z = function () {
					...
					for (var ab = 0; ab < T.length; ab++) {
						...
						if (V && V.padding != undefined)
							ac.push('padding:' + m(V.padding));
						if (c && b.quirks && W[ab].align)
							ac.push('text-align:' + W[ab].align);
						if (ac.length > 0)
							aa.push('style="', ac.join('; '), '" ');
	...
	k.dialog.uiElement.prototype = {
		...
		selectParentTab : function () {
			...
			if (U._.dialog._.currentTabId != T)
				U._.dialog.selectPage(T);
			return U;
		},
		focus : function () {
		...
		registerEvents : function (R) {
			...
			for (var V in R) {
				if (!(T = V.match(S)))
					continue;
				if (this.eventProcessors[V])
					this.eventProcessors[V].call(this, this._.dialog, R[V]);
				else
					U(this, this._.dialog, T[1].toLowerCase(), R[V]);
			}
			return this;
		},
		eventProcessors : {
	...
	k.dialog.hbox.prototype = e.extend(new k.dialog.uiElement(), {
			getChild : function (R) {
				...
				if (arguments.length < 1)
					return S._.children.concat();
				if (!R.splice)
					R = [R];
				if (R.length < 2)
					return S._.children[R[0]];
				else
					return S._.children[R[0]] && S._.children[R[0]].getChild ? S._.children[R[0]].getChild(R.slice(1, R.length)) : null;
			}
		}, true);
		...
		a.dialog.validate = {
			functions : function () {
				...
				return function () {
					...
					if (ac < X.length && typeof X[ac] == 'number')
						aa = X[ac];
	...
	e.extend(a.editor.prototype, {
		openDialog : function (R, S) {
			...
			function Z(ab) {
				...
				ad = Y.skin.dialog;
				if (!ad._isLoaded || aa && typeof ab == 'undefined')
					return;
				if (typeof ac != 'function')
					a.dialog._.dialogDefinitions[R] = 'failed';
				Y.openDialog(R, S);
			};
			if (typeof U == 'string') {
				var aa = 1;
				...
				showBlock : function (p, q, r, s, t) {
					...
					e.setTimeout(function () {
						var E = e.bind(function () {
								var F = w.getFirst();
								if (v.autoSize) {
									var G = v.element.getDocument(),
									H = (b.webkit ? v.element : G.getBody()).$.scrollWidth;
									if (c && b.quirks && H > 0)
...
(function () {
	...
	function r(D) {
		...
		if (E(G) || E(H, 1))
			return true;
		if (!(G || H) && !(F.type == 1 && F.isBlockBoundary() && F.getBogus()))
			return true;
		return false;
	};
	var s = {
		...
		exec : function (D) {
			...
			case 'source':
				var E = D.textarea.$;
				if (c)
					E.createTextRange().execCommand('SelectAll');
				else {
					E.selectionStart = 0;
	...
	function v(D) {
		var E = D && u(D);
		if (E)
			if (E.getCustomData('ready'))
				w(D);
			else
				E.setCustomData('ready', 1);
	};
	function w(D) {
		var E = D && D.removeCustomData('cke-fillingChar');
		if (E) {
			var F,
			G = D.getSelection().getNative(),
			...
			if (b.webkit) {
				...
				function G() {
					...
					if (J) {
						...
						if (K.type == 'Caret' && K.anchorNode == J.$)
							F = 1;
						E = J.getText();
			...
			D.on('contentDom', function () {
				...
				if (c) {
					...
					K.on('focusin', function (V) {
						if (V.data.$.srcElement.nodeName != 'BODY')
							return;
					...
					if (I.$.compatMode != 'BackCompat') {
						...
						if (b.ie8) {
							...
							function S() {
								...
								W = V.createRange();
								if (V.type != 'None' && W.parentElement().ownerDocument == I.$)
									W.select();
							};
						}
					}
	...
	d.selection = function (D) {
		...
		if (c)
			try {
				...
				if (!F || F.item && F.item(0).ownerDocument != G.document.$ || F.parentElement && F.parentElement().ownerDocument != G.document.$)
					throw 0;
			} catch (H) {
				G.isInvalid = true;
			}
	...
	d.selection.prototype = {
		...
		getType : c ? function () {
			...
			try {
				...
				if (G == 'Control')
					E = 3;
				if (F.createRange().parentElement)
					E = 2;
			} catch (H) {}
		 ...
		 : function () {
			...
			if (!F)
				...
				H = G.startContainer;
				if (H == G.endContainer && H.nodeType == 1 && G.endOffset - G.startOffset == 1 && y[H.childNodes[G.startOffset].nodeName.toLowerCase()])
					E = 3;
			}
			return D.type = E;
		},
		getRanges : (function () {
			var D = c ? (function () {
					...
					return function () {
						...
						if (I == 2) {
							...
							if (J.endContainer.getPosition(J.startContainer) & 4 && J.endOffset <= J.startContainer.getIndex())
								J.collapse();
							return [J];
						} else if (I == 3) {
							var L = [];
			...
			return function (E) {
				...
				if (F.ranges && !E)
					...
					F.ranges = new d.rangeList(D.call(this));
				if (E) {
					var G = F.ranges;
					for (var H = 0; H < G.length; H++) {
						...
						if (I.startContainer.isReadOnly()) {
							...
							while (K) {
								if (K.is('body') || !K.isReadOnly())
									break;
								if (K.type == 1 && K.getAttribute('contentEditable') == 'false')
									I.setStartAfter(K);
								K = K.getParent();
							}
						}
				...
				if (G) {
					if (!G.collapsed) {
						...
						while (1) {
							...
							I = G.startOffset;
							if (I == (H.getChildCount ? H.getChildCount() : H.getLength()) && !H.isBlockBoundary())
								G.setStartAfter(H);
							else
								break;
						}
		...
		getSelectedElement : function () {
			var D = this._.cache;
			if (D.selectedElement !== undefined)
			...
			F = e.tryThese(function () {
					...
					if (G) {
						...
						P = function (Q, R) {
							return function (S, T) {
								if (S.type == 3 && (!e.trim(S.getText()) || S.getParent().data('cke-bookmark')))
									return true;
								var U;
								if (S.type == 1) {
									U = S.getName();
									if (U == 'br' && R && S.equals(S.getParent().getBogus()))
										return true;
									if (T && U in N || U in f.$removeEmpty)
										return true;
								}
								Q.halted = 1;
								return false;
					...
					H,
					I;
					for (var J = 2; J && !((H = G.getEnclosedNode()) && H.type == 1 && y[H.getName()] && (I = H)); J--)
						G.shrink(1);
					return I.$;
				});
		...
		getSelectedText : function () {
			var D = this._.cache;
			if (D.selectedText !== undefined)
				return D.selectedText;
			var E = '',
			F = this.getNative();
			if (this.getType() == 2)
				E = c ? F.createRange().text : F.toString();
		...
		selectRanges : function (D) {
			...
			if (c) {
				...
				for (var G = 0; G < D.length; G++) {
					...
					O = M.startContainer;
					if (M.collapsed && (b.opera || b.gecko && b.version < 10900) && O.type == 1 && !O.getChildCount())
						O.appendText('');
	...
	d.range.prototype.select = c ? function (D) {
		...
		if (O.startContainer.type == 1 && O.startContainer.getName()in C || O.endContainer.type == 1 && O.endContainer.getName()in C)
			O.shrink(1, true);
		...
		if (L) {
			...
			G.insertBefore(K);
			if (F)
				O.document.createText('\ufeff').insertBefore(K);
		}
		O.setStartBefore(K);
		K.remove();
		if (E) {
